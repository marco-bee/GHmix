...
#' 3. the maximum of the product of the univariate kernel densities
#' @param ABCsam (m x k) matrix: ABC sample, where m is the ABC sample size and k is the
#' number of parameters.
#' @return A list containing the 3 approximate modes.
...
AMLEmode <- function(ABCsam)
{
  ...
  # 3. maximum of the product of the univariate kernel densities
  ...
  res = list(SampleMean = AMLE1, UnivariateKD = AMLE2, ProductUnivariateKD = AMLE4)
  return(res)
}
